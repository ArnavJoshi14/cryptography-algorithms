...
def generate_keys(key):
    ...
    for i in range(16):
        ...
        keys.append(permute(left + right, PC2, 48))
    return keys

def des_round(text, keys, decrypt=False):
    text = permute(text, IP, 64)
    left, right = text[:32], text[32:]
    if decrypt:
        keys = keys[::-1]
    for i in range(16):
        exp = permute(right, E, 48)
        x = xor(exp, keys[i])
        s = ""
        for j in range(8):
            block = x[j*6:(j+1)*6]
            ...
            col = int(block[1:5], 2)
            s += bin(S_BOX[j][row][col])[2:].zfill(4)
        p = permute(s, P, 32)
        left, right = right, xor(left, p)
...
def encrypt(pt, key):
    ...
    pt = pad(pt)
    hx = binascii.hexlify(pt.encode()).decode()
    out = ""
    for i in range(0, len(hx), 16):
        out += bin2hex(des_round(hex2bin(hx[i:i+16]), keys))
    return out
...
def decrypt(ct, key):
    ...
    for i in range(0, len(ct), 16):
        hx += bin2hex(des_round(hex2bin(ct[i:i+16]), keys, True))
    return binascii.unhexlify(hx).decode(errors="ignore")

if __name__ == "__main__":
    pt = input("Enter Plaintext: ")
    key = input("Enter 16-char Hex Key: ") or "133457799BBCDFF1"
    ct = encrypt(pt, key)
    print("Encrypted:", ct)
