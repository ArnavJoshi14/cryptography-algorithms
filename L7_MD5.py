...
def md5(msg):
    ...
    orig_len = (len(msg) * 8) & 0xffffffffffffffff
    ...
    msg += orig_len.to_bytes(8, 'little')
    ...
    for i in range(0, len(msg), 64):
        ...
        for j in range(64):
            ...
            w = int.from_bytes(block[4*g:4*g+4], 'little')

            temp = (a + f + K[j] + w) & 0xFFFFFFFF
            new_b = (b + ((temp<<s[j]) | (temp>>32-s[j]))) & 0xFFFFFFFF
            a, b, c, d = d, new_b, b, c
    ...
    return sum((x<<32*i) for i,x in enumerate([A, B, C, D])).to_bytes(16, 'little').hex()

print(md5(input("enter msg: ")))
