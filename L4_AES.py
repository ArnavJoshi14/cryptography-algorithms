...
def add_round_key(state, round_key):
    return [[state[r][c] ^ round_key[c][r] for c in range(4)] for r in range(4)]
...
def key_expansion(key):
    ...
    return [key_words[i:i+4] for i in range(0,44,4)]
...
def main():
    ...
    key_input = "abcdefghijklmnop"
    plaintext_bytes = user_input.encode('utf-8')
    key_bytes = key_input.encode('utf-8')
    padded_plaintext = pad(plaintext_bytes)
    key_list = list(key_bytes)
    round_keys = key_expansion(key_list)
    encrypted_bytes = []
    for i in range(0, len(padded_plaintext), 16):
        block = list(padded_plaintext[i:i+16])
        encrypted_bytes.extend(aes_encrypt_block(block, round_keys))
    ...
    print("Decrypted:", unpad(bytes(decrypted_bytes)).decode('utf-8'))
